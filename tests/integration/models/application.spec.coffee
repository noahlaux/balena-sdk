...
describe 'Application Model', ->
	...
	describe 'given a multicontainer application with a single offline device', ->
		...
		describe 'balena.models.application.getWithDeviceServiceDetails()', ->
			...
			it 'should retrieve devices with service details', ->
				balena.models.device.getWithDeviceServiceDetails(@application.id).then (applicationDetails) =>
					m.chai.expect(applicationDetails.owns_device[0]).to.deep.match
					...
					m.chai.expect(applicationDetails.owns_device[0].current_services.db).to.have.lengthOf(1)

					# Should have an empty list of gateway downloads
					m.chai.expect(applicationDetails.owns_device[0].current_gateway_downloads).to.have.lengthOf(0)